...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (body.mode === "individual") {
    ...
    return jsonResponse(
      { message: "Iscrizione individuale registrata.", teamId: data?.id, name: data?.name },
      { status: 201 },
    );
  }
  ...
  if (body.mode === "team") {
    ...
    if (!teamName || !contactEmail) {
      return jsonResponse({ message: "Il nome del team e l'email di contatto sono obbligatori." }, { status: 400 });
    }

    if (members.length < 2 || members.length > 4) {
      return jsonResponse({ message: "Il team deve avere da 2 a 4 membri." }, { status: 400 });
    }
    ...
    return jsonResponse(
      { message: "Team registrato con successo.", teamId: data?.id, name: data?.name },
      { status: 201 },
    );
  }
