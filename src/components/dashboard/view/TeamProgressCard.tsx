    ...
    <Card className="flex h-full min-h-0 w-full flex-col">
      <CardHeader className="pb-0">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="font-semibold text-lg text-neutral-900">Progressi del team</CardTitle>
            <CardDescription className="text-neutral-500 text-sm">
              Avanzamento verso la challenge e stato delle submission.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="min-h-0 flex-1 space-y-4 pt-3">
          ...
          {progressItems.map(({ id, label, value, total, percentage }) => (
            <div key={id} className="space-y-2 p-2">
              ...
              {typeof percentage === "number" && (
                <Progress value={percentage} className="h-2 rounded-full bg-neutral-100" />
              )}
            </div>
          ))}
        </div>
        ...
        <Alert className="flex items-start gap-3 border-amber-100 bg-amber-50/80 text-amber-700">
          ...
          <AlertDescription className="text-amber-700 text-xs">
            Leggi le istruzioni prima di procedere: errori di formato possono causare rifiuti.
          </AlertDescription>
        </Alert>
